...
export const UsersFindFirstSchema = Joi.object().keys({ where: Joi.object().keys(UsersWhereInputSchemaObject), orderBy: Joi.object().keys(UsersOrderByWithRelationInputSchemaObject), cursor: Joi.object().keys(UsersWhereUniqueInputSchemaObject), take: Joi.number(), skip: Joi.number(), distinct: Joi.array().items(UsersScalarFieldEnumSchema) }).required()